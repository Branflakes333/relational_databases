...
SELECT COALESCE(null,null,'student',null,5);
...
SELECT nullif(1,1);
SELECT nullif(1,'student');

------------------------------------------------------------
-- Ex 4.
-- GREATEST examples
------------------------------------------------------------
select GREATEST(10,2,4,6);
select GREATEST(10,2,4,60);
select GREATEST(10,2,null,60);

------------------------------------------------------------
-- Ex 5.
-- LEAST examples
------------------------------------------------------------
select LEAST(10,2,4,6);
select LEAST(10,20,4,60);
select LEAST(10,2,null,60);


------------------------------------------------------------
-- Ex 6.
-- 6A. Return rows  from epa_site_location where it is in 'Butte', 
...
------------------------------------------------------------
-- 6B. Return rows where its daily_mean_pm10_concentration is higher than 
...
------------------------------------------------------------
-- Ex 7. Return rows in epa_site_location 
...
------------------------------------------------------------
-- Ex 8. Return site_id, minimum, average and maximum daily_mean_pm10_concentration 
...
------------------------------------------------------------
-- Ex 9. Return date, site_name, daily_mean_pm10_concentration and daily_aqi_value ordered by site_id and date.
------------------------------------------------------------
SELECT date, 
	   site_name,
...
------------------------------------------------------------
-- Ex 10. Assuming that data was supposed to be collected from every station on all the dates in epa_air_quality.
...
------------------------------------------------------------
-- Ex 11 . We are interested in how many readings were collected per site quarterly every year.
...
------------------------------------------------------------
-- Ex 12 . Return the number of site_ids for each county and also return each individual site_name sorted by count name.
-- The output should have county, num_site_ids and array of site_names. This output should be sorted by county names.
------------------------------------------------------------
SELECT county, COUNT(DISTINCT site_id) AS num_site_ids, 
